...
class Database
{
    ...
    public function pridajPouzivatela(Pouzivatel $pouzivatel) {
        ...
        $statement->execute([$pouzivatel->getMeno(), $pouzivatel->getPriezvisko(), $pouzivatel->getMail(),$pouzivatel->getMesto(), $pouzivatel->getUlica(), $pouzivatel->getHeslo()]);
    }
    ...
    public function upravInfoPouzivatela(Pouzivatel $pouzivatel) {
        ...
        $stmt->execute([$pouzivatel->getMeno(), $pouzivatel->getPriezvisko(), $pouzivatel->getMail(), $pouzivatel->getMesto(), $pouzivatel->getUlica(), $pouzivatel->getDruhyMail()]);
    }
    ...
    public function dajVsetkyProdukty() {
        $sql = $this->pdo->prepare("SELECT DISTINCT idProduktu, cestaKObrazku, nazovProduktu FROM produkty PR JOIN varianty_produktov VP ON PR.idProduktu = VP.id_produktu && VP.pocetKusov != 0");
        $sql->execute();
        return $sql->fetchAll(PDO::FETCH_CLASS, PonukanyProdukt::class);
    }
